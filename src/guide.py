...
def guide_page():
    ...
    st.image(supervised_thumbnail, caption="Supervised Learning", use_container_width=True)
    if st.button("Watch Supervised Learning"):
        try:
            st.video(supervised_video_path, format="mp4")
        except Exception as e:
            st.error(f"Error while loading the Supervised Learning video: {str(e)}")

    st.subheader("2. Unsupervised Learning")
    st.image(unsupervised_thumbnail, caption="Unsupervised Learning", use_container_width=True)
    if st.button("Watch Unsupervised Learning"):
        try:
            st.video(unsupervised_video_path, format="mp4")
        except Exception as e:
            st.error(f"Error while loading the Unsupervised Learning video: {str(e)}")

    st.subheader("3. Deep Learning")
    st.image(deep_learning_thumbnail, caption="Deep Learning", use_container_width=True)
    if st.button("Watch Deep Learning"):
        try:
            st.video(deep_learning_video_path, format="mp4",)
        except Exception as e:
            st.error(f"Error while loading the Deep Learning video: {str(e)}")
    ...
    # Additional text explaining the sections of the app
